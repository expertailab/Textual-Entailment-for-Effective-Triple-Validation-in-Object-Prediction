...
def recall(preds: List[str], gts: List[List[str]]) -> float:
    ...
    # When the ground truth object is none return 1 even if there
    # are predictions (edge case)
    if is_none_gts(gts):
        return 1.0
...
def main():
    ...
    scores_per_sr_pair = evaluate_per_sr_pair(args.predictions, args.ground_truth)
