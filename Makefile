...
ifeq ($(OS),Windows_NT)
    RM = del /F /Q
    RMDIR = -RMDIR /S /Q
    RM_WILD := python -c "$(RMDIR_WILD_WIN_FN)"
    MKDIR = -mkdir
...
clean-build:
	...
	$(RMDIR) dist/
	$(RMDIR) .eggs/
	$(RM_WILD) *.egg-info *.egg

## Remove Python file artefacts
clean-pyc:
	$(RM_WILD) **__pycache__ *.pyc *.pyo
...
lint:
	isort --lbt 1 --ls lm_kbc
	black lm_kbc --include '.py' --exclude '.ipynb'
	black lm_kbc --include '.ipynb' --ipynb
	flake8 lm_kbc
...
docs:
	$(RMDIR) docs/lm_kbc.rst
	$(RMDIR) docs/modules.rst
	sphinx-apidoc -o docs/ lm_kbc
	$(MAKE) -C docs clean
...
ifneq (,$(wildcard requirements.txt))
	conda run --name $(PROJECT_NAME) pip install -r requirements.txt
	conda run --name $(PROJECT_NAME) python -c "import nltk; nltk.download('punkt')"
	conda run --name $(PROJECT_NAME) python -m spacy download en_core_web_trf
endif
ifneq (,$(wildcard requirements-dev.txt))
	conda run --name $(PROJECT_NAME) pip install -r requirements-dev.txt
endif
	...
	source venv/bin/activate; pip install -r requirements.txt -r requirements-dev.txt; python -c "import nltk; nltk.download('punkt')"; python -m spacy download en_core_web_trf
